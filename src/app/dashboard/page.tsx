...
export default async function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-qcc-dark">
          Welcome, {profile?.full_name || 'Faculty'}
        </h1>
        {profile?.department && (
          <p className="text-qcc-gray mt-1">{profile.department}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          ...
          <p className="text-sm text-qcc-gray">Courses Completed</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <p className="text-3xl font-bold text-qcc-sky">{badges?.length || 0}</p>
          <p className="text-sm text-qcc-gray">Badges Earned</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <p className="text-3xl font-bold text-green-600">
            {completions?.filter(c => c.verified_by).length || 0}
          </p>
          <p className="text-sm text-qcc-gray">Verified</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
          <p className="text-3xl font-bold text-amber-600">
            {(pathways?.length || 0) - (badges?.length || 0)}
          </p>
          <p className="text-sm text-qcc-gray">Pathways In Progress</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <h2 className="text-xl font-semibold text-qcc-dark mb-4">My Badges</h2>
          {badges && badges.length > 0 ? (
            <div className="space-y-3">
              {badges.map((badge) => (
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-qcc-dark text-sm">{badge.pathway?.badge_name}</p>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 text-center">
              <p className="text-qcc-gray text-sm">No badges earned yet.</p>
          ...
          {pathways && pathways.length > 0 && (
            <div className="mt-6">
              <h3 className="text-lg font-semibold text-qcc-dark mb-3">Pathway Progress</h3>
              <div className="space-y-3">
                {pathways.map((pathway) => {
                  ...
                  return (
                    <Link
                      ...
                      className="block bg-white border border-gray-200 rounded-xl p-4 hover:border-qcc-sky transition-colors"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <p className="font-medium text-qcc-dark text-sm">{pathway.title}</p>
                        {earned && (
                      ...
                      <p className="text-xs text-qcc-gray mt-1">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-qcc-dark">Completed Courses</h2>
          ...
          {completions && completions.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-medium text-qcc-dark text-sm truncate">{completion.course?.title}</p>
                      <p className="text-xs text-qcc-gray">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-8 text-center">
              ...
              <p className="text-qcc-gray text-sm mb-2">No courses completed yet.</p>
